...
Ground-up scratchwork for conversations about what an accessibility-first mathematical workspace should be.
...
The repository contains a deliberately small Electron prototype for local mathematical documents, casually called “math napkins” in the UI. It is scratchwork meant to generate conversation about an accessibility-first mathematical workspace—not a starting point that contributors are expected to extend. It is not a finished mathematical editor, and the automated test suite is not evidence that the workflow is usable without feedback from blind users.
...
- [Open example napkins](#opening-editable-example-napkins)
...
Omniya Core is being built for and with blind mathematicians and STEM students, screen-reader users, accessibility and assistive-technology specialists, MathML/LaTeX/speech/Braille experts, interaction researchers, educators, developers, and technical writers.
...
This project is not trying to retrofit a conventional visual editor with labels and keyboard shortcuts. The prototype is intentionally scratch work: a small, disposable instrument for asking better design questions before committing to a larger architecture.
...
The point of the Electron app is to make these hypotheses testable. It should be easy to replace or discard parts of it when research with blind users exposes a better model. Major decisions should be discussed, tested, and revised with the people who rely on the resulting experience—not inferred by sighted developers from visual convenience alone.
...
The code is intentionally disposable. This prototype should be treated as a conversation prompt and source of concrete questions, not as a product foundation or a finished replacement for existing mathematical software.
...
The highest-priority contribution right now is conversation with the community about what an accessibility-first mathematical workspace should be. The existing client is only a concrete prompt for that conversation; it is not the foundation or interaction model the project has already chosen.
...
Assistive-technology testing, documentation, MathML review, interaction experiments, usability-study design, and focused bug fixes are useful when they support that discussion. Contributions do not need to preserve the current UI or architecture.
...
### What makes a strong issue or pull request

Start with the user problem and affected workflow. Describe the expected and actual behavior; operating system, screen reader, runtime, and input method; exact napkin content or reproduction steps; the affected workflow; and tests or a focused reproduction when possible.

Changes should preserve offline behavior, semantic HTML, native MathML, and understandable focus behavior. Include or update tests for behavioral changes, keep the scope focused, and explain intentional document-model changes.
...
Omniya Core is an early prototype maintained by Axiya and released under the Apache License 2.0. The current implementation is intentionally small, experimental, and subject to change. It should be evaluated as a conversation prompt for ground-up research with blind mathematicians, not as a finished replacement for existing mathematical software.

The repository does not yet include a formal `CONTRIBUTING.md`, issue templates, pull-request template, accessibility testing guide, or research-questions document. Adding those would be valuable follow-up work as the project begins to receive contributions.
