...
| Typeforms and punctuation | Rules 7, 8 | local letter/number typeform modes, bounded Rule 7.3.4–7.3.5 mathematical-expression open/close scopes now have loaded-Electron creation/edit evidence, reviewed punctuation tokens including the two-cell short dash (8.7), bounded plural `s`, and the bounded punctuation-indicator + apostrophe + s suffix used after a script (8.4/14.13) implemented; ordinary plural letters remain an explicit local choice against the letter `s`; phrase-level non-mathematical text and punctuation-indicator placement after every required indicator remain pending | exact official examples, focused whole/subexpression Braille projections, scope closure, source-linked diagnostics, and the named Electron typeform workflow |
...
| Scripts | Rule 14 | one-level scripts, all two- and three-component level-indicator chains from Rules 14.4.2–14.4.3, all 16 four-component chains from Rule 14.4.4, numeric script operands, first compound msubsup transitions, context-safe baseline promotion to MathML multiscripts/mprescripts, Rule 14.5 left-subscript entry and right-script composition, Rule 14.7 contracted script commas, Rule 14.12 prime-before-script ordering, bounded Rule 14.13 possessive suffixes, and nested script/radical composition are implemented as local operations; Rule 14.4.4’s “more than three” construction is exercised to depth 32 by repeated registered local script operations, not by a general level parser; the remaining full prescript fixture corpus remains pending | scripts, two/three/four-level direction chains, nested depth stress, contracted commas, primes, possessive suffix, left/right prescripts, multiscripts and baseline return |
| Modifiers | Rule 15 | five-step directly-over/under transitions, bounded multi-token modifier scopes, simultaneous under/over conversion, same-side higher-order wrapping (15.3), parallel-bar local rows (15.5), binomial table construction (15.6), the complete published Rule 15.12 arrow-modifier catalog as bounded local sequences, contracted modified-script compositions (15.7), modifier catalog, and integral superposition follow-ups implemented; a loaded-Electron overbar creation/edit workflow is now verified; plural modified expressions (15.8) remain tied to the Rule 8.4 suffix review | modifiers on atoms, grouped expressions, every Rule 15.12 arrow variant, higher-order nesting, parallel bars, binomial, Rule 15.7 whole/focused subscript fixtures, nested scopes, and the named Electron overbar workflow |
...
| Shapes and functions | Rules 17, 18 | source-listed abbreviated function names and upper/lower limit forms are bounded atomic rows; Rule 17.6.2 horizontal and 17.6.3 vertical multiple-interior circle constructions are bounded atomic rows with source intent; loaded-Electron diamond/square construction and `sin` application editing are verified; drawn-in shapes, full function spacing/punctuation, numeric function subscripts, and open-ended unlisted names remain ordinary compositional or pending context policies | named and abbreviated functions; complete multi-interior shape rows, independent SRE projection, and the named Electron geometry/function workflows |
...
| Miscellaneous symbols | Rule 23 | reviewed literal symbols include single, repeated, lower, and upper integrals, the four Rule 23 listed circle/infinity/rectangle/square integral superpositions, monetary units, at/crossed symbols, quantifiers, relation symbols, Greek variants, and the transcriber-defined QED shape; Rule 23.10 degree is a bounded superscript decoration (`~.*`), not a free token; serializer-only integral glyph aliases were removed; remaining symbol-index and context/spacing constructions remain pending | symbol-index fixtures, focused integral and degree fixtures, and errata rows |
