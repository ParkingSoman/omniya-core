...
| Scripts | Rule 14 | one-level scripts, all two- and three-component level-indicator chains from Rules 14.4.2–14.4.3, numeric script operands, first compound msubsup transitions, context-safe baseline promotion to MathML multiscripts/mprescripts, Rule 14.7 contracted script commas, Rule 14.12 prime-before-script ordering, and bounded Rule 14.13 possessive suffixes implemented; deeper-than-three levels, other possessive contexts, and full prescript fixture corpus pending | scripts, every two/three-level direction chain, contracted commas, primes, possessive suffix, prescripts, multiscripts and baseline return |
