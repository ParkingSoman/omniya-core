...
| Letters and numerals | Rules 3, 5, 6 | English, Greek, the German glyphs printed in the BANA 6.1.1 table, Hebrew aleph, Russian ell/sha, lower-cell Nemeth numerals, bounded non-decimal-base digits (3.6), uppercase Roman numeral local mode (3.11), capitalization, and bounded alphabet indicators implemented; ordinal, ordinary plural, possessive, and table/diagram contexts remain document or context-policy work | number indicator, base digits, Roman sequence, capitalization, non-English alphabet, and invalid-context fixtures |
| Typeforms and punctuation | Rules 7, 8 | local letter/number typeform modes, reviewed punctuation tokens, and the bounded punctuation-indicator + apostrophe + s suffix used after a script (8.4/14.13) implemented; ordinary plural letters, phrase-level typeform boundaries, punctuation-indicator placement after every required indicator, and word/expression scope remain pending | exact official examples and source-linked diagnostics |
...
| Fractions | Rule 13 | simple, complex, hypercomplex, and mixed structural openings, fraction-line transitions, and terminators are source-linked to the published indicators; continued-fraction and spatial-fraction forms are pending/deferred with Rule 25 | nested fraction depths 2, 8, 32 and terminators |
| Scripts | Rule 14 | superscript/subscript, numeric script operands, first compound msubsup transitions, context-safe baseline promotion to MathML multiscripts/mprescripts, Rule 14.7 contracted script commas, Rule 14.12 prime-before-script ordering, and bounded Rule 14.13 possessive suffixes implemented; deeper levels, other possessive contexts, and full prescript fixture corpus pending | scripts, contracted commas, primes, possessive suffix, prescripts, multiscripts and baseline return |
| Modifiers | Rule 15 | five-step directly-over/under transitions, bounded multi-token modifier scopes, simultaneous under/over conversion, same-side higher-order wrapping (15.3), parallel-bar local rows (15.5), binomial table construction (15.6), modifier catalog, and integral superposition follow-ups implemented; modified-script combinations remain pending | modifiers on atoms, grouped expressions, higher-order nesting, parallel bars, binomial, and nested scopes |
| Radicals | Rule 16 | square, indexed, cube, and fourth-root transitions retain the published `>`, `<n>`, `/`, and `]` local codes | nested radicals and index slots |
...
| Comparisons and arrows | Rules 21, 22 | source-backed core relations and official directional/diagonal/shaft examples implemented; guessed arrow combinations removed pending a reviewed bounded component registry; modified and vertically compounded comparison catalog remains pending | direct BANA source fixtures, ambiguity and compound comparison fixtures |
| Miscellaneous symbols | Rule 23 | reviewed literal symbols include single, repeated, lower, and upper integrals, integral superposition follow-ups, monetary units, at/crossed symbols, quantifiers, relation symbols, Greek variants, and the transcriber-defined QED shape; remaining symbol-index and context/spacing constructions remain pending | symbol-index fixtures, focused integral fixtures, and errata rows |
