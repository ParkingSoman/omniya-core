...
| Rule 23.10 degree | BANA's worked degree example uses the direct-over indicator before the hollow dot: `~.*`. | `misc.degree` is a bounded `wrap-script-token` decoration. On a populated focus it wraps the base in an `msup`; on an empty draft it opens an owned base hole. It is never a free `mo` token, and remains distinct from standalone hollow-dot `.*`. |
