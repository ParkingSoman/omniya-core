...
| Direct Nemeth fraction workflow | `Nemeth fraction creation and MathJax numerator navigation edit preserve the denominator` creates `x/y` with `⠹⠭⠌⠽` | The same workflow navigates to `Numerator x` and replaces it with `z` | Exact `<mfrac>` children, whole `⠹⠭⠌⠽⠼`, focused `⠭`, and post-edit `⠹⠵⠌⠽⠼` |
| Cancellation and omission scope | `Nemeth cancellation owns its content and MathJax edits only the canceled term` creates `⠪⠭⠻` from an owned cancellation hole | The same workflow navigates into the canceled `x` and replaces it with `z` | Owned `<menclose>` content, whole `⠪⠭⠻`/`⠪⠵⠻`, focused `⠭`, and preserved cancellation wrapper |
...
| Left scripts and prescripts | `Nemeth left-subscript choice creates multiscripts and MathJax edits the owned base` enters the Rule 14.5 left-subscript code, selects its local meaning, and fills the base | The same workflow uses Explorer Down to reach `Base n` and replaces only the base | Explicit ambiguity choice, canonical `<mmultiscripts>`, whole `⠰⠭⠐⠝`/`⠰⠭⠐⠵`, focused `⠝`, and preserved left subscript |
| Typeform scopes | `Nemeth typeform scope creation and MathJax inner editing preserve the bold MathML scope` opens BANA 7.3.5 bold scope, enters `a+b`, and closes it | The same workflow navigates into the scoped `a` and replaces it with `z` | Canonical `<mstyle mathvariant="bold">`, whole `⠁⠬⠃`/`⠵⠬⠃`, focused `⠁`, exact inner replacement, and retained scope |
| Comparisons and relation operators | `Nemeth comparison creation and MathJax relation navigation edit preserve both operands` creates `x < y` from separate local cells | The same workflow navigates root → relation → right operand and replaces only `y` | Relation MathML, whole `⠭⠀⠐⠅⠀⠽`/`⠭⠀⠐⠅⠀⠵`, focused `⠽`, and preserved left operand/relation |
| Modifiers and overbars | `Nemeth modifier creation and MathJax base navigation edit preserve the overbar scope` enters `x`, holds the Rule 15 bar cell, and commits the bounded local modifier | The same workflow uses Explorer Down to focus the overbar base and replaces only `x` with `z` | `<mover>` ownership, whole `⠭⠱`/`⠵⠱`, focused `⠭`, exact base replacement, and preserved overscript |
| Functions and application | `Nemeth function creation and MathJax argument navigation edit preserve application structure` commits the bounded Rule 18 `sin` code, then enters `x` as a separate token | The same workflow navigates root → application → argument and replaces only `x` with `z` | Function atomic Enter boundary, application MathML, whole/focused Braille, exact argument replacement, and restored application focus |
| Integrals and operator follow-ups | `Nemeth integral creation and MathJax sign navigation edit preserve the local follow-up structure` enters ordinary `⠮`, then the registered repeated-integral follow-up `⠮` | The same workflow enters Explorer, navigates to the focused integral sign, and replaces the exact sign with ordinary `⠮` | Whole `⠮⠮` creation projection, structural-follow-up clearing, focused sign Braille, exact post-edit `⠮`, and no ancestor replacement |
| Integral bounds as generic script composition | `Nemeth integral bounds are created locally and MathJax navigation edits only the selected bound` creates `∫` immediately, then uses Rule 14 subscript and superscript follow-ups with `⠰⠁⠘⠃` | The same loaded workflow uses Explorer root → lower bound, presses E, and replaces only `a` with `z` | Canonical `<msubsup>`, whole `⠮⠰⠁⠘⠃`, focused lower-bound `⠰⠁`, exact post-edit `⠮⠰⠵⠘⠃`, and restored focus |
| Degree decoration as a bounded script token | `Nemeth degree decoration is created as one local code and MathJax edits its base only` enters `90` with the numeric cells, holds `~.*` (`⠘⠨⠡`) until Enter, and creates the `msup` degree object | The same loaded workflow uses MathJax Explorer after creation, presses E at the focused base, and replaces only `90` with `d`, preserving the degree overscript | No MathML mutation before local Enter, canonical `<msup>` children `90`/`°`, whole `⠼⠔⠴⠘⠨⠡`, focused base Braille, exact post-edit `⠙⠘⠨⠡`, and restored focus |
| Geometry and literal shapes | `Nemeth geometry atom creation and MathJax whole-scope editing preserve the local code` commits the bounded Rule 17 diamond construction | The same workflow freezes the whole Explorer scope and replaces it with the bounded square construction | Atomic Enter boundary, exact `<mo>` replacement, whole/focused Braille, and no stale diamond |
...
| Input equivalence | `six-key input feeds the same Nemeth draft transition as Unicode cells` creates `l` with a real `F+S+D` chord while the replacement draft is open | The same loaded-Electron test enters MathJax Explorer, moves to `l`, presses E, submits a second six-key chord, and verifies the exact replacement and focused Braille | Unicode Braille, Braille ASCII, display input, and six-key simulation reach identical transition results and saved MathML; creation and editing both cross the renderer boundary |
