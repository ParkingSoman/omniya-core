...
const rows = inventory.rows.map((row) => {
  ...
  const mappingIds = registry.filter((mapping) => mapping.banaRefs?.includes(ref) || (isExample && mapping.banaRefs?.includes(example?.sourceRows?.[0]))).map((mapping) => mapping.id);
