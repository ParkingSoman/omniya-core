...
lines.push('', '## Row-level canonical ledger', '', '| Order | Rule | Source row | Title / short scope | Page | Official notation / whole Braille | Input policy | Disposition | Registry / code | Unit evidence | Electron creation | Electron editing | Navigation | Visual evidence | Transcriber review | Blind-contributor review | Verification |', '| ---: | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |');
for (const row of rows) {
  const rule = ruleFor(row);
  ...
  lines.push(`| ${row.auditOrder} | ${ruleAnchor}Rule ${rule}: ${ruleNames[rule] ?? 'Other'} | ${source} | ${short(row.title)} | ${row.printedPage ?? row.pdfPage ?? '—'} | ${officialCells(row)} | ${policy(row)} | ${row.disposition} | ${mappings} ${row.mappingIds?.length ? codeLink : ''} | ${row.unitCaseIds?.length ? unitLink : '—'} | ${linkEvidence(row, 'electronCreation')} | ${linkEvidence(row, 'electronEditing')} | ${row.navigationCaseIds?.length ? electronLink : '—'} | ${visualLink} | ${row.humanReview?.transcriber?.status ?? 'pending'} | ${row.humanReview?.blindContributor?.status ?? 'pending'} | ${check(row)} |`);
}
