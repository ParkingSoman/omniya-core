...
function renderReport({ cases, sourceIds, totals, bySource, refuseCauseRows, disagrees, errors }) {
  ...
  push('DISAGREE below has been checked and falls into one of five sub-causes:');
  ...
  push('together and left no cell at the seam), `homograph` (the Nemeth Code itself writes two');
  push('different print signs with one set of cells), or `glyph-spelling` (the Code and the');
  push('corpus write the same sign with two different Unicode codepoints).');
  push();
